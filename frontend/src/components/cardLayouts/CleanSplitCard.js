    ...
    return (
      <Box sx={{
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.8 }}>
          {performer.age && (
            <Typography sx={{ fontWeight: 'bold', fontSize: '0.9rem', color: 'var(--text)' }}>
              {displayAge}
            </Typography>
          )}
          {countryFlag && (
  ...
  if (mode === 'filter') {
    return (
      <Box sx={{
        ...
        '&:hover': { borderColor: 'var(--accent-quiet)', boxShadow: '0 6px 24px var(--accent-quiet)', transform: 'translateY(-3px)' }
      }} onClick={onClick}
        ...
        <Box sx={{ p: 1.5 }}>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <Box sx={{ display: 'flex', gap: 0.5, flexShrink: 0 }}>
              ...
              {onOpenHash && (
                <IconButton onClick={(e) => { e.stopPropagation(); if (performer.latest_internal_run_id) window.open(`/hash-results/${performer.latest_internal_run_id}`, '_blank'); else onOpenHash(performer.id); }} sx={{ ...iconSx, color: 'var(--info)' }}>
          ...
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mt: 0.3 }}>
            ...
            <Typography sx={{ color: 'var(--muted)', fontSize: '0.65rem' }}>⬇ {daysSinceImport !== null ? `${daysSinceImport}d` : '—'}</Typography>
          </Box>
          {/* Progress bars */}
          <Box sx={{ mt: 0.5 }}>
            {[
              { l: 'Pics', v: picsPercentage, c: 'var(--ok)', type: 'pics' },
              { l: 'Vids', v: vidsPercentage, c: 'var(--info)', type: 'vids' },
              ...
              <Box key={p.l} onClick={(e) => { e.stopPropagation(); if (onProgressClick) onProgressClick(performer, p.type); }}
                ...
                <Typography sx={{ color: 'var(--dim)', fontSize: '0.65rem', width: 26 }}>{p.l}</Typography>
                <Box sx={{ flex: 1, height: 6, bgcolor: 'var(--surface)', borderRadius: 3, overflow: 'hidden' }}>
    ...
    >
      <Box sx={{ flex: 1, position: 'relative', overflow: 'hidden' }}>
        ...
        <Box onClick={handleRatingBadgeClick} sx={{
          ...
          color: 'var(--warn)', fontWeight: 'bold', fontSize: '0.85rem', cursor: 'pointer'
        }}>⭐ {ratingValue !== null ? formatRating(ratingValue) : 'Rate'}</Box>
        <Box sx={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, var(--bg) 0%, transparent 25%)' }} />
      </Box>
      <Box sx={{ p: 1.5 }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 0.5 }}>
          ...
          <Box sx={{ display: 'flex', gap: 0.5 }}>
            {onSettings && <IconButton onClick={(e) => { e.stopPropagation(); onSettings(performer); }} sx={{ ...iconSx, color: 'var(--info)' }}><SettingsIcon /></IconButton>}
